...
public class Scanner {
	...
	public void scan() throws Exception {
		...
		if (logging)
			Imgcodecs.imwrite("D:\\step_6_canny.png", adaptiveThresh);
		findParentRectangle();
		...
		for (int index = 0; index < answers.size(); index++) {
			...
			System.out.println((index + 1) + ". " + (optionIndex == null ? "EMPTY/INVALID" : options[optionIndex]));
		}

		Imgcodecs.imwrite("D:\\result.png", source);
	}
	...
	private void findParentRectangle() throws Exception {
		...
		if (logging)
			System.out.println("getParentRectangle > hiearchy data:\n" + hierarchy.dump());
		...
		MatOfPoint cTest[]=new MatOfPoint[rectangles.size()];
		int i=0;
		for (Map.Entry<Double, MatOfPoint> rectangle : rectangles.entrySet()) {
			double index = rectangle.getKey();
			cTest[i]=contours.get((int)index);
			i++;
		}
		
		for(i=0;i<cTest.length;i++)
		{
			double max=Imgproc.contourArea(cTest[i]);
			for(int j=i+1;j<cTest.length;j++)
			{
				double t=Imgproc.contourArea(cTest[j]);
				if(t>max)
				{
					max=t;
					MatOfPoint tmp=cTest[i];
					cTest[i]=cTest[j];
					cTest[j]=tmp;
				}
			}
		...
		for(i=0;i<cTest.length;i++)
		{
			roi = boundingRect(cTest[i]);
			Imgcodecs.imwrite("D:\\abc"+String.valueOf(i)+".png", source.submat(roi));
...
//            double childId = ids[2];
		...
		if (logging)
			System.out.println("getParentRectangle > modified roi.x: " + roi.x + ", roi.y: " + roi.y);
		if (logging)
			System.out.println("getParentRectangle > modified roi.width: " + roi.width + ", roi.height: " + roi.height);

		if (logging)
			Imgcodecs.imwrite("D:\\step_7_gray.png", source.submat(roi));
	}
	...
	private void findBubbles() throws Exception {
		...
		if (logging)
			System.out.println("findBubbles > ideal circle size > minThreshold: " + minThreshold + ", maxThreshold: "
					+ maxThreshold);
		...
		Util.sortTopLeft2BottomRight(drafts);
...
//                if(logging) write2File(conjuction, "conjuction_" + i + "_" + j + ".png");
			...
			if (logging)
				System.out.println(
						"recognizeAnswers > selection is " + (selection == null ? "empty/invalid" : selection[2]));
...
//                putText(source.submat(roi), "(" + i + "_" + selection[2] + ")", new Point(rows.get(selection[2]).get(0, 0)), Core.FONT_HERSHEY_SIMPLEX, 0.3, new Scalar(0, 255, 0));
	...
	private void drawText(Mat img, Point ofs, String text) {
		Imgproc.putText(img, text, ofs, Imgproc.FONT_HERSHEY_PLAIN, 0.5, new Scalar(255, 255, 25));
