...
public interface ISubjectComponentRepository<T> {
	...
	PaginationResult<T> searchByTeacher(int teacherId, String filter, String key, int start, int size);

	PaginationResult<T> searchBySubject(int subjectId, String filter, String key, int start, int size);
}
