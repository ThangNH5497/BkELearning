...
public class ExamPaperRepositoryImpl extends AbstractGenericRepository<ExamPaper> implements IExamPaperRepository {
	...
	@Override
	public PaginationResult<ExamPaper> getByTeacherRepoAndSubject(String teacherFilter, String subjectFilter, int start,
			int size) {
		Session session = sessionFactory.getCurrentSession();
		PaginationResult<ExamPaper> page = new PaginationResult<ExamPaper>();
		try {
			String hqlQuery = "FROM ExamPaper ep " + " WHERE ";
			// get by admin
			...
			Query q = session.createQuery(
					"SELECT new ExamPaper(ep.id,ep.code,ep.name,ep.user,ep.subject,ep.descriptor,ep.time,ep.createAt,ep.updateAt) "
							+ hqlQuery);
	...
	@Override
	public PaginationResult<ExamPaper> searchByTeacherRepoAndSubject(String teacherFilter, String subjectFilter,
	...
	public PaginationResult<ExamPaper> getByAdminRepoAndSubject(String subject, int start, int size) {
		...
		PaginationResult<ExamPaper> page = new PaginationResult<ExamPaper>();
		try {
			String hqlQuery = "FROM ExamPaper ep " + " WHERE ep.bankType=:bankType ";
			// get by admin
			...
			if (!subject.equals("ALL")) {
				q.setParameter("subjectId", Integer.parseInt(subject));
				qCount.setParameter("subjectId", Integer.parseInt(subject));
			}
			page.setCount((Long) qCount.uniqueResult());
	...
	public PaginationResult<ExamPaper> searchByAdminRepoAndSubject(String subject, String key, int start, int size) {
		...
		try {
			String hqlQuery = "FROM ExamPaper ep "
					+ " WHERE ep.bankType=:bankType and (ep.code like concat('%',:key,'%') or ep.name like concat('%',:key,'%')) ";
			// get by admin
